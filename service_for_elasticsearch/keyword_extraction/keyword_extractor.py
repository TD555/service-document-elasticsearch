    ...
    while i < len(doc):
        ...
        if lang == 'ru':
            ...
            if doc[i].dep_ in ["amod", "compound"]:
                ...
                if ' ' in comp_text.strip() and comp_text.strip() and not re.compile(et_al_pattern).match(comp_text.strip()) and not all(token.strip().lower() in sw_s_en for token in comp_text.split()) and all(len(token.strip()) > 1 for token in comp_text.split()):
                    compound_nouns[comp_text.strip().title()] += 1
                    i += 1
                    continue
