...
SEARCH_URL = os.environ['SEARCH_URL'] + "?db=pubmed&term={keyword}&retmode=json&retmax={limit}&retstart={offset}"
FETCH_URL = os.environ['FETCH_URL'] + "?db=pubmed&id={id}&rettype=medline&retmode=xml"
...
async def fetch_with_retry(session, id, retry_attempts=10, timeout=10):
    for attempt in range(retry_attempts):
        try:
            async with session.get(FETCH_URL.format(id=id.strip()) + f"&api_key={PUBMED_API_KEY}", timeout=timeout) as response:
...
async def search_with_retry(session, keyword, limit, offset, retry_attempts=10, timeout=10):
    for attempt in range(retry_attempts):
        try:
            async with session.get(SEARCH_URL.format(keyword=keyword, limit=limit, offset=offset) + f"&api_key={PUBMED_API_KEY}", timeout=timeout) as response:
                response.raise_for_status()
