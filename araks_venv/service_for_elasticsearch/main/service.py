...
async def check_elements(my_list):
    return all(isinstance(x, int) and 0 <= x <= 100 for x in my_list)
...
async def match(source, target, method, score=100):
    if method == "fuzzy":
        if type(target[1]) == list and len(target[1]) > 1:
            results = await asyncio.gather(*[fuzzy_similar(await list_to_str(source[1]), trg) for trg in target[1]])
            ratio = max(results)

            max_index = results.index(ratio)

            source_value = list_to_str(source[1])
            target_value = target[1][max_index]

        else:
            ratio = await fuzzy_similar(await list_to_str(source[1]), list_to_str(target[1]))

            source_value = list_to_str(source[1])
            target_value = list_to_str(target[1])

        if ratio >= score:
            # print(ratio)
            return {"nodes": (source[0], target[0]), "values": (source_value, target_value)}

    elif method == "exact":
        if type(target[1]) == list and len(target[1]) > 1:
            ratio = any([await exact_similar(await list_to_str(source[1]), trg) for trg in target[1]])
        else:
            ratio = await exact_similar(await list_to_str(source[1]), target[1])
        if ratio:
            return {"nodes": (source[0], target[0]), "values": (source[1], target[1])}
...
select_deafault_script = "SELECT id AS node_id, name AS value from nodes where project_type_id = '{type_id}'"
...
async def fuzzy_match():
    ...
    try:
        ...
        pntp_ids = data['pntp_ids']
        methods = data['methods']
        scores = data['scores']
        source_default_prop = data['source_default_prop']
        target_default_prop = data['target_default_prop']
        has_rel = data['has_rel']
        rel_id = data['rel_id']

    except Exception as e:
        error_message = "Invalid JSON data: " + str(e)
        # return make_response(jsonify({"error": error_message, "status_code" : 400}), 400)
        abort(400, error_message)

    if len(type_ids) != 2:
        abort(400, "Type IDs should consist of two values.")

    if not await check_elements(scores):
        abort(400, "Scores must be whole numbers within the range of 70 to 100.")

    if len(scores) != len(methods) or len(scores) != len(pntp_ids['source']) or len(scores) != len(pntp_ids['target']):
        abort(400, "Lengths must be equal to each other.")
    ...
    try:
        conn = ps.connect(dbname="araks", user="postgres",
                          password="tik555", port="5433", host="localhost")
        ...
        for i in range(len(pntp_ids['source'])):
            ...
            if pntp_ids['source'][i] not in datas:
                if source_default_prop == pntp_ids['source'][i]:
                    cur.execute(select_deafault_script.format(
                        type_id=type_ids[0], property_id=source_default_prop))

                else:
                    cur.execute(select_props_script.format(
                        type_id=type_ids[0], property_id=pntp_ids['source'][i]))
            ...
            if pntp_ids['target'][i] not in datas:
                if target_default_prop == pntp_ids['target'][i]:
                    cur.execute(select_deafault_script.format(
                        type_id=type_ids[1], property_id=target_default_prop))

                else:
                    cur.execute(select_props_script.format(
                        type_id=type_ids[1], property_id=pntp_ids['target'][i]))
            ...
            for src_item in datas[pntp_ids['source'][i]]:
                ...
                for trg_item in datas[pntp_ids['target'][i]]:
                    ...
                    if src_item[0] != trg_item[0]:
                        result = match(
                            src_item, trg_item, methods[i], scores[i])

                        tasks.append(result)


    except Exception as e:
        error_message = "Error while matching: " + str(e)
        abort(400, error_message)
    ...
    return [{'source': result[0], 'target': result[1]} for result in results]
